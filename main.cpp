// Taken help from https://aishack.in/tutorials/sudoku-grabber-opencv-extracting-grid/
...
int main(void)
{
    Mat sudoku = imread("sudoku-3.jpg", 0);

    if (!sudoku.data)
    {
        cout << "Could not open or find the sudoku" << std::endl;
        return -1;
    }
    ...
    // imshow("Finding the biggest blob", outerBox);

    // detecting the lines
    vector<Vec2f> lines;
    HoughLines(outerBox, lines, 1, CV_PI / 180, 200);
    mergeRelatedLines(&lines, sudoku); // Add this lin
    ...
    warpPerspective(sudoku, undistorted, cv::getPerspectiveTransform(src, dst), Size(maxLength, maxLength));

    // getting the digits from the main sudoku image
    ...
    for (int j = 0; j < 9; j++)
    {
        for (int i = 0; i < 9; i++)
        {
            ...
            {
                // we can print the digit extracted using imshow
                // classifying the digit using the trained data so far
